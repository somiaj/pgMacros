...
=item C<S<< UseSolutions => 0 >>>

If this flag is set to an array of solutions, force the list
of solutions to be the given list and do not generate solutions.
This option is slightly different than providing the solutions
below, in that it will use the solutions for both the correct
answer and the student's answer.

Context()->flags->set(
    UseSolutions => [[1,2], [3,4], [5,6]]
);


This avoids having to generate solutions for both the answer
key and the students answer, but makes the grader less accurate
as it will only test if the student's answer satisfies the given
solutions and will not test if the answer key satisfies solutions
to the student's answer. For example if the answer was the top
half a circle, and the student entered in an equation for the
full circle, it would be accepted.

=item C<S<< GenerateFunction => 0 >>>

If this is set to a subroutine, use the subroutine to generate
solutions to the equation instead of default generation method.
The subroutine is sent a single input, the number of the solution
to generate (starting at 1).

Example to generate solutions to x^2 + y^2 = 16:

Context()->flags->set(
    GenerateFunction => sub {
        my $n = shift;
        my $x = random(-4,4,0.25);
        if ($n < 6) {
            return [$x, sqrt(16 - $x**2)];
        } else {
            return [$x, -sqrt(16 - $x**2)];
        }
    }
...
This function returns points on the top half of the circle for the first
5 points, and points on the bottom half of the circle for the next 5 points.

Note, this method is less robust than the default method, since it will
only generate solutions to the answer key, and not generate solutions for
the student's answer. This means that the only check is if the student's
equation satisfies the generated solutions and not visa versa. For example if
the desired answer was the top half of a circle, but the student entered
in an equation for the full circle, this grader would still mark the student
correct. Using this improves efficiency at the cost of possible accuracy.
...
sub createPoints {
	...
	#
	#  Use provided solution list if it exists.
	#
	my $useSolutions = $self->getFlag('UseSolutions', 0);
	if (ref($useSolutions) eq 'ARRAY') {
		$self->{solutions} = $useSolutions;
		return;
	}

	#
	#  Use function to generate points if it is provided.
	#
