...
sub getCorrectAns {
	...
	my %choice;
	@choice{ @{ $self->{choices} } } = 1 .. scalar @{ $self->{choices} };
...
sub PRINT_BUTTONS {
	...
	while (@buttons) {
		...
		$count++;
		push(@out,
			qq!<label><input type="$buttonType" name="$name" id="${name}$countstr" aria-label="${label}option $count" value="$value" $checked>$item</label>!
		);
	}
	my $out_str = join("\n" . $self->{separator}, @out);
	return $out_str;
...
sub cmp_defaults { (shift->SUPER::cmp_defaults(@_), partialCredit => $main::showPartialCorrectAnswers,) }
